...
if __name__ == "__main__":
    ...
    datestamp = str(25) + str(month_to_num(config['file_prefix'][-3::])) + config['file_prefix'][0:2]  # TURN-ON # MLPG: remove
    ...
    centroid = False #config['centroid']
    if args.no_centroid:
        centroid = False

    centroider = config['centroider']
    sami_centroider = False # config['sami_centroider']
    savestatdata = False # config['savestatdata']
    ...
    object_tile_file = Path(config['data_dir']) / f"raw/{datestamp}" / Path(config['robot_file_name'].replace("Robot", "Tile").replace("_CorrectionsApplied", ""))
    ...
    tile_file.close()
    tile_name = config['robot_file_name'].replace("Robot", "Tile").replace("_CorrectionsApplied", "")[0:-4]
    ...
    for i in range(np.size(hector_ccd)):
        ...
        if data_type == "reduced":
            object_file_Hector = Path(config['data_dir']) / f"reduced/{datestamp}/{tile_dir}/{tile_dir}_F0/main/" / f"ccd_{hector_ccd[i]}" / f"{config['file_prefix']}{hector_ccd[i]}{obs_number:04}red.fits"
            object_file_AAOmega = Path(config['data_dir']) / f"reduced/{datestamp}/{tile_dir}/{tile_dir}_F0/main/" / f"ccd_{aaomega_ccd[i]}" / f"{config['file_prefix']}{aaomega_ccd[i]}{obs_number:04}red.fits"
        else:
    ...
    for Probe in plist:
        ...
        # scat_plt1.append(ax.add_collection(utils.display_ifu_nofill(x_rotated, y_rotated, mean_x, mean_y, scale_factor, Probe_data, color='k')))
        ...
        """ 
            ***** Calculations of the Optical model corrections based on the polynomial function *****
        """
        # Polynomial coefficients from the fitting done in "process_quicklook_files"
        prPurple(f"Polynomial Coefficients from the fitting done in 'process_quicklook_files'")
        coeffs = [-5.10579247e-03, -9.08942022e-01, 1.94332118e+02]
        polynomial = np.poly1d(coeffs)

        probeX, probeY = mean_x, mean_y
        plateX, plateY = robot_centre_in_mm[1], robot_centre_in_mm[0]

        radial_plate_to_probe = np.sqrt( (plateX - probeX) ** 2.0 + (plateY - probeY) ** 2.0 )

        """ Use the polynomial function to estimate the correction """
        # Note: In the polynomial coeff estimation, I used scaled the radial_plate_to_probe by 1E3, which is applied
        #       consistently below.
        RPerpenDist_estimated = polynomial(radial_plate_to_probe / 1.0E3)   * scale_factor
        radial_plate_to_RPerpenDistPoint = np.sqrt( (RPerpenDist_estimated ** 2.0) + (radial_plate_to_probe ** 2.0) )

        names = ['Probe', 'MeanX', 'MeanY', 'Probe_maxRadius', 'RotationAngle', 'CentroidX_rotated',
                 'CentroidY_rotated', 'CentroidXErr_rotated', 'CentroidYErr_rotated', 'CentroidX_COM_rotated', 'CentroidY_COM_rotated',
                 'CentroidRMS_Err', 'RotationAngle_Centroid', 'RadialDist', 'RadialDistErr', 'PDist', 'QDist',
                 'PDistErr','QDistErr', 'NDist', 'EDist', 'NDistErr', 'EDistErr', 'RPerpenDist', 'RParallelDist', 'RPerpenDistErr',
                 'RParallelDistErr', 'TelecentricAng', 'RadialDist_Plate', 'RadialDist_PlateErr']
        centroid_data = pd.read_csv(glob.glob(f"{save_files}/Hector_fit/*.csv")[0], names=names, delimiter=',', skiprows=1)

        centroid_marker = plt.scatter(centroid_data['CentroidX_rotated'] * scale_factor + centroid_data['MeanX'],
                                      centroid_data['CentroidY_rotated'] * scale_factor + centroid_data['MeanY'],
                                      s=15, c='b', marker='x', zorder=300, linewidth=0.5)
        scat_plt.append(ax.add_artist(centroid_marker))


        probe_data = centroid_data[centroid_data['Probe']==Probe]
        if not probe_data.empty:
            prCyan(f"For Probe {Probe}, the Radial RPerpenDist Correction (polynomial-based): {RPerpenDist_estimated} (a big number because of it's scaled by {scale_factor} ) "
                   f"vs the RPerpenDist: {probe_data['RPerpenDist'].values[0]} ")


        from sympy import symbols, Eq, solve

        x, y = symbols('x,y')
        eq1 = Eq((plateX - x) ** 2.0 + (plateY - y) ** 2.0, radial_plate_to_RPerpenDistPoint ** 2.0)
        eq2 = Eq((probeX - x) ** 2.0 + (probeY - y) ** 2.0, RPerpenDist_estimated ** 2.0)

        result_nonlinear = solve([eq1, eq2], (x, y))
        prGreen(f"For Probe {Probe}: The Rperpendicular X,Y coordinates: {result_nonlinear}")

        # Check angles to figure out the
        ...
        angle1, sign1  = calculate_angles((plateX, plateY), result_nonlinear[0])
        angle2, sign2  = calculate_angles((plateX, plateY), result_nonlinear[1])

        if sign1 == sign2: # Either both angles are positive or negative
            if RPerpenDist_estimated < 0:
                if angle1 > angle2: new_point = result_nonlinear[0]
                else: new_point = result_nonlinear[1]
            else:
                if angle1 < angle2: new_point = result_nonlinear[0]
                else: new_point = result_nonlinear[1]

        else: # A special case where one angle is positive and the other negative - happens if the probe is @0 deg. or @180 deg
            minIndex_sign  = np.argmin([sign1, sign2])
            angles_degrees = np.array([Math.degrees(angle1), Math.degrees(angle2)])
            angles_degrees[minIndex_sign] = 360. + angles_degrees[minIndex_sign]
            maxIndex, minIndex = np.argmax(angles_degrees), np.argmin(angles_degrees)
            prRed(f"Special case where the angles are +/-ve in radians ({angle1, angle2}) and in degrees {angles_degrees}")

            if RPerpenDist_estimated < 0:
                if 180.< angles_degrees[maxIndex] <=270.: new_point = result_nonlinear[maxIndex]  # If the angle belongs to the third quadrent and RpernpenDist is negative (i.e. clockwise), we pick the smallest angle
                else: new_point = result_nonlinear[minIndex]
            else:
                if 90. < angles_degrees[minIndex] <= 180.:new_point = result_nonlinear[minIndex]  # If the angle belongs to the third quadrent and RpernpenDist is negative (i.e. clockwise), we pick the smallest angle
                else: new_point = result_nonlinear[maxIndex]

        prGreen(f"\nFor Probe {Probe}: the new X,Y coordinates: {np.array(new_point)/1.0E3} (in robot coor. - center_y/center_x)\n")

        scat_plt1.append(ax.add_collection(utils.display_ifu_nofill(x_rotated, y_rotated, new_point[0], new_point[1], scale_factor, color='grey')))
