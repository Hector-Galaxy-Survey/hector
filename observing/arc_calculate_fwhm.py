...
#version 0.5: 30th April 2025 - update median FWHM based on 2022 - 2024 measurements and Simpson method for Gaussian fitting
#Susie Tuntipong
...
def read_wavelengths(reduced_arc_path):
    ...
    return wavelengths, wavelength_per_pixel
...
def Gaussian_func_0(x_measured,multiple,x_standard,sigma):
    '''Gaussian function'''
...
def Gaussian_func(x_measured,multiple,x_standard,sigma):
    '''Gaussian fitting with Simpson method'''

    xx1 = x_measured - (wavelength_per_pixel/2)
    xx2 = x_measured
    xx3 = x_measured + (wavelength_per_pixel/2)

    yy1 = Gaussian_func_0(xx1,multiple,x_standard,sigma)
    yy2 = Gaussian_func_0(xx2,multiple,x_standard,sigma)
    yy3 = Gaussian_func_0(xx3,multiple,x_standard,sigma)

    yy = (1/6)*(yy1+(4*yy2)+yy3)
    return yy
...
def fitting(reduced_arc_path, outdir):
    ...
    for r in range(len(reduced_arc_path)):
        ...
        if len(pos_framename)!=0:
            ...
            #read wavelengths
            wavelength_all, wavelength_per_pixel = read_wavelengths(reduced_arc_path[r])
...
def plot_fwhm(outdir):
    ...
    colour = ['b','r','b','r']
    lower_lim = [np.array([2.2,1.2,1.0,0.8]),[35,35,35,35],[1500,600,-0.5,-0.5],[2000,2000,-0.5,-0.5],[100,400,2700,2200]]
    upper_lim = [np.array([3.2,2.2,2.0,1.8]),[45,45,45,45],[3000,2500,0.5,0.5],[3000,3000,0.5,0.5],[200,600,3000,2500]]
    median_all = np.array([2.55,1.52,1.40,1.20])
